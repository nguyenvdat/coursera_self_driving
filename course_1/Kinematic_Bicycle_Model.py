...
class Bicycle(Bicycle):
    def step(self, v, w):
        ...
        self.xc += self.sample_time * v * np.cos(self.theta + self.beta)
        self.yc += self.sample_time * v * np.sin(self.theta + self.beta)
        if w > self.w_max:
            w = self.w_max
        self.delta += self.sample_time * w
...
for i in range(t_data.shape[0]):
    x_data[i] = model.xc
    y_data[i] = model.yc
    if i < 0.95 / 8 * t_data.shape[0]: 
        if model.delta < np.arctan(2/8):
            w = model.w_max
        else:
            w = 0
    elif i < 4.9 / 8 * t_data.shape[0]:
        if model.delta > -np.arctan(2/8):
            w = -model.w_max
        else:
            w = 0
