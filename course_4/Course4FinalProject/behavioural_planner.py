...
class BehaviouralPlanner:
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        while wp_index < len(waypoints) - 1:
            arc_length += np.linalg.norm(np.array(waypoints[wp_index + 1][0:2]) - np.array(waypoints[wp_index][0:2]))
            if arc_length > self._lookahead:
                return wp_index
            wp_index += 1
        # ------------------------------------------------------------------
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        wp_x, wp_y = waypoints[i][0], waypoints[i][1]
        d = np.sqrt((ego_x - wp_x) ** 2 + (ego_y - wp_y) ** 2)
        if d < closest_len:
            closest_len = d
            closest_index = i
    # ------------------------------------------------------------------
