    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                path_f = np.array([paths[i][0][-1], paths[i][1][-1]])
                score = np.linalg.norm(path_f - np.array(goal_state[0:2]))
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            # d = np.array(paths[j][0:2]) - np.array(paths[i][0:2])
                            # score += self._weight * np.mean(np.sqrt(np.sum(d * d, axis=0)))
                            collision_path_f = np.array([paths[j][0][-1], paths[j][1][-1]])
                            score += -np.linalg.norm(path_f - collision_path_f)
                            # --------------------------------------------------
